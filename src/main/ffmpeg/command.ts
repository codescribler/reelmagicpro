...
// Slow-mo smoothing via frame BLENDING. `setpts` only stretches time (it holds
// each source frame), so 0.5× on a 30fps source shows ~15 motion updates/sec —
// visibly jerky. `minterpolate` in blend mode fills the gaps by averaging
// neighbouring frames, adding natural motion blur that masks the stepping
// WITHOUT the warping/morphing artifacts motion-compensated interpolation
// produces on fast action (football). Targets the SOURCE frame rate so every
// exported part stays at one frame rate (keeping the sequence concat valid) and
// file size stays modest. Only applied for slow-mo (speed < 1); '' otherwise.
// Must be appended AFTER setpts so it blends the already-slowed stream.
function slowmoBlend(speed: number, fps: number): string {
  if (speed >= 1) return '';
  return `,minterpolate=fps=${fmt(fps)}:mi_mode=blend`;
}
...
export function buildClipFfmpegArgs(
  ...
  const videoFilter = `[0:v]crop=${fmt(width)}:${fmt(height)}:${fmt(x)}:${fmt(y)},scale=${source.width}:${source.height}${markerFilters ? ',' + markerFilters : ''},${watermark}${brightnessFilter(clip.brightness)},setpts=${setpts}${slowmoBlend(clip.speed, source.fps)}[v]`;
...
//   crop(focus-box zoom) → scale(srcW:srcH)        // apply the clip's zoom
//   → [optional] markers (post-zoom space)
//   → crop(cropSide × cropSide, full height, at x(t))   // square reel slice
//   → scale(1080:1080)
//   → pad(1080:1920, black bars top/bottom)
//   → IG watermark → brightness → setpts → [v]
//
// Reels apply the focus-box zoom first (so the reel framing is recorded and
// applied within the zoomed frame), then the square reel slice pans across that
// post-zoom frame. `samples` carry the crop rect over time in POST-ZOOM source
// pixels (the frame is back at srcW×srcH after the zoom scale). cy/w/h are
// constant (square, centred); only cx varies. crop re-evaluates x per frame
...
export function buildInstagramClipFfmpegArgs(
  ...
  const setpts = clip.speed === 1 ? 'PTS-STARTPTS' : `(PTS-STARTPTS)/${fmt(clip.speed)}`;
  // Markers are drawn in post-zoom space (buildMarkerFilters maps marker coords
  // through the same zoom), matching the standard export.
  const markerFilters = buildMarkerFilters(clip, source);
  ...
  const videoFilter = `[0:v]crop=${fmt(z.width)}:${fmt(z.height)}:${fmt(z.x)}:${fmt(z.y)}`
    ...
    + `,setpts=${setpts}${slowmoBlend(clip.speed, source.fps)}[v]`;
